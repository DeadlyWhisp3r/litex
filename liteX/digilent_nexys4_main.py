...
class BaseSoC(SoCCore):
    def __init__(self, sys_clk_freq=75e6,
        ...
        **kwargs):
        ...
        # UART -------------------------------------------------------------------------------------
        #if with_uart: ###change to with_my_uart
        from litex.soc.cores.uart import UARTPHY
        from litex.soc.cores.uart import UART
        uart_pads = self.platform.request("my_uart_pads")
        uart_phy = UARTPHY(uart_pads, clk_freq=sys_clk_freq, baudrate=115200)
        uart_instance = UART(uart_phy, tx_fifo_depth=1, rx_fifo_depth=16)
        #uart = UARTCrossover(tx_fifo_depth=1, rx_fifo_depth=16, rx_fifo_rx_we=True)
        self.add_module("my_uart",uart_instance)
